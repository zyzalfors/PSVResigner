class PSVResigner {
    ...
    hidden static [byte[]] $PS2Key = (0xEA, 0x02, 0xCE, 0xEF, 0x5B, 0xB4, 0xD2, 0x99, 0x8F, 0x61, 0x19, 0x10, 0xD7, 0x7F, 0x51, 0xC6)
    ...
    hidden static [byte[]] GetSignature([byte[]] $data, [int] $type) {
        ...
        elseif($type -eq 2) {
            ...
            $salt = [PSVResigner]::InvokeAES("CBC", [PSVResigner]::PS2Key, [PSVResigner]::IV, $salt, $true)
        }
        else {
            return $null
        }
